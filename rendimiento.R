...
producto=function(based){
  for (i in 1:dim(based)[1]) {
    ...
    if (length(grep("GRANO",based$Nombre.del.producto.de.interés.económico.obtenido[i]))){
      ...
      based$Nombre.del.producto.de.interés.económico.obtenido[i]="OTRO (ESPECIFIQUE)"
    }
    ...
    else if (length(grep("NINGUNO|SINIESTRADO|PERDIDA|NO HUBO|NO HYA|NO HAy|NO SE COSECHO|NADA|NO SE TUVO|NO SE OBTUVO|SEGURO AGRICOLA|MANOJO|PLANTA COMPLETA|BIOMASA|DESARROLLO DE LA PLANTA|EN PIE|PAS 540|RICINUS|ABONO VERDE|HUITLACOCHE",based$Nombre.del.producto.de.interés.económico.obtenido[i]))){
      based$Nombre.del.producto.de.interés.económico.obtenido[i]=NA
    }
  }
  return(based)
